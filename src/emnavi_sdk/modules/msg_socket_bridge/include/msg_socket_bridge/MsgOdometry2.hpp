#ifndef __MSG_ODOMETRY2_
#define __MSG_ODOMETRY2_
// for OTHER_ODOM
...
class MsgOdometry2:public BaseMsg<nav_msgs::OdometryPtr,nav_msgs::Odometry,BUF_LEN_SHORT>
{
private:
    std::string drone_id_str_;
    /* data */
public:
    MsgOdometry2(ros::NodeHandle &nh,TOPIC_ENUM msg_type,int send_fd,struct sockaddr_in send_addr,std::string send_topic,std::string recv_topic)
    : BaseMsg(nh,msg_type,send_fd,send_addr,send_topic,recv_topic) {}
    void set_drone_id(int id)
    {
        drone_id_str_ = std::string("drone_") + std::to_string(id);
    }
    int serialize(const nav_msgs::OdometryPtr &msg)
    {
        ...
        // child_frame_id
        ptr += serialize_one_string(drone_id_str_,ptr);
        // header
