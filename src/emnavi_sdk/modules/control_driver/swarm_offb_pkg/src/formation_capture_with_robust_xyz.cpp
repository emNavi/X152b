...
float f = 20;
float g = 2;
float diff_step = 1/ROS_RATE;

Eigen::Matrix2d A = (Eigen::Matrix2d() << -g, 0, 1, -f).finished();
Eigen::Vector2d B_u(1,0);
Eigen::Vector2d B_x( (-g * g), (f + g));

float u_all_x;
float u_all_y;
float u_all_z;

Eigen::Vector2d x_r_x(0, 0);
Eigen::Vector2d x_r_y(0, 0);
Eigen::Vector2d x_r_z(0, 0);

Eigen::Vector2d ode45_simulator_o2(Eigen::Vector2d &x_r, Eigen::Matrix2d &A, Eigen::Vector2d &B_x,
                       Eigen::Vector2d &B_u, double x_state, double u_all)
{
    Eigen::Vector2d k_1, k_2, k_3, k_4;
    k_1 = A * x_r + B_u * u_all + B_x * x_state;
    k_2 = A * (x_r + diff_step / 2 * k_1) + B_u * u_all + B_x * x_state;
    k_3 = A * (x_r + diff_step / 2 * k_2) + B_u * u_all + B_x * x_state;
    k_4 = A * (x_r + diff_step * k_3) + B_u * u_all + B_x * x_state;
    Eigen::Vector2d x_new = x_r + diff_step / 6 * (k_1 + 2 * k_2 + 2 * k_3 + k_4);
    return x_new;
}
...
{
    cur_pos = *msg;
    cur_pos.pose.position.x = cur_pos.pose.position.x + bias_x;
    cur_pos.pose.position.y = cur_pos.pose.position.y + bias_y;
}
...
geometry_msgs::Vector3Stamped robustController(geometry_msgs::PoseStamped expected_pose,
                                               geometry_msgs::TwistStamped expected_velocity,
                                               geometry_msgs::Vector3Stamped expected_accel)
{
    float err_x = cur_pos.pose.position.x - expected_pose.pose.position.x;
    ...
    x_r_x = ode45_simulator_o2(x_r_x, A, B_x, B_u, err_x, u_all_x);
    x_r_y = ode45_simulator_o2(x_r_y, A, B_x, B_u, err_y, u_all_y);
    x_r_z = ode45_simulator_o2(x_r_z, A, B_x, B_u, err_z, u_all_z);

    float urc_x = f * g * (x_r_x[1] - err_x);
    float urc_y = f * g * (x_r_y[1] - err_y);
    float urc_z = f * g * (x_r_z[1] - err_z);

    u_all_x = unc_x + urc_x;
    u_all_y = unc_y + urc_y;
    u_all_z = unc_z + urc_z;

    ctrl_accel.vector.x = u_all_x;
    ctrl_accel.vector.y = u_all_y;
    ctrl_accel.vector.z = u_all_z;
    return ctrl_accel;
}
        ...
        else
        {
            if (!takeoff_flag)
            {
                ...
                default:
                    ...
                    exp_acc.vector.y = 0;
                    exp_acc.vector.z = 0;
                    ctrl_accel = robustController(exp_pos, exp_vel, exp_acc);
                    break;
                case '1':
